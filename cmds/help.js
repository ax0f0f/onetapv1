...
const helpData = {
  ...
  setup: {
    ...
    commands: [
      ...
      { name: '.v auto-setup', value: 'Automatically set up the ax0f category with One Tap voice and interface channels.' },
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    const titleText = new TextDisplayBuilder().setContent('# ax0f Help Center');

    const descriptionText = new TextDisplayBuilder().setContent(
      `> **axf0**  creates smooth, secure, and instant temporary voice channels on Discord`
    );
    ...
    collector.on('collect', async interaction => {
      ...
      const categoryFooter = new TextDisplayBuilder().setContent(`*Developed by ichbi9o • Use the menu to switch categories.*`);
